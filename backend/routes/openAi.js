...
module.exports.post = async (req, res) => {
  ...
  try {
    ...
    // Check OpenAI count
    const openaiCountupdate = await db.models.OpenAiCount.findOne({
      email: email,
    });
    if (!openaiCountupdate) {
      const newOpenaiCount = await db.models.OpenAiCount.create({
        email: email,
        count: 1,
      });
    }
    ...
    if (openaiCountupdate) {
      const updatedOpenaiCount = await db.models.OpenAiCount.updateOne(
        { email: email },
        { count: openaiCountupdate.count + 1 }
      );
    }
